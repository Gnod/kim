...
import com.ashampoo.kim.format.tiff.constant.FujiFilmTag
...
class FujiFilmSimulationTest {
    ...
    fun testFilmSimulationExtraction() {
         ...
         */
        println("Extracted Film Simulation: ${photoMetadata.filmSimulation}")
    ...
    fun testNonFujiFilesHaveNoFilmSimulation() {
        ...
        if (cameraMake?.contains("FUJIFILM", ignoreCase = true) != true) {
            val photoMetadata = metadata.convertToPhotoMetadata()
            assertNull(photoMetadata.filmSimulation, "Non-Fuji files should not have film simulation")
        }
    }
